	...
	let i = Random.int n and j = Random.int n in
	  if i = j then loop g m
		...
		(fun label ->
		   ...
		   then { label with Scs.eq = label.Scs.eq + 1}
		   else { label with Scs.ne = label.Scs.ne + 1})
		g i j { Scs.eq = 0; Scs.ne = 0 }
	    in
	      loop g (m - 1)
    in
      loop g m in
  let g =
	...
	let i = Random.int n and j = Random.int n in
	  if i = j then loop g k
	    ...
	    let { Scs.eq = eq; Scs.ne = ne } =
	      ELGraph.get_label_default g i j { Scs.eq = 0; Scs.ne = 0 }
	    in
	      if IntMap.get c i = IntMap.get c j
	      then
		if eq > 0
		then loop (ELGraph.set_label g i j { Scs.eq = eq - 1; Scs.ne = ne + 1}) (k - 1)
		else loop g k
	      else
		if ne > 0
		then loop (ELGraph.set_label g i j { Scs.eq = eq + 1; Scs.ne = ne - 1}) (k - 1)
